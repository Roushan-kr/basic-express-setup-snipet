...
const createUser = asyncHandler(async (req, res) => {
  const { userName, fName, email, password, role, semester, section } =
    req.body;
...
const updateStudentInfo = asyncHandler(async (req, res) => {
  ...
  if (role === "student") {
    if (!semester || !section) {
      return res
        .status(400)
        .json({ message: "Semester and Section are required for students." });
    }
  }
  ...
  if (checkUser) {
    throw new ApiError(400, [
      "Email or Username already in use by another user.",
    ]);
  }
...
const delUserWithPass = asyncHandler(async (req, res) => {
  const userId = req.user._id;
